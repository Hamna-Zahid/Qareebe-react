...
const ShopOnboarding3_Details = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            data.append('name', completeData.shopName);
            data.append('location', `${completeData.area}, ${completeData.city}`); // Concatenate for simple location string
            data.append('address', completeData.address);
            data.append('deliveryTime', completeData.deliveryTime);
            data.append('description', completeData.description);
            data.append('deliveryFee', completeData.deliveryFee);

            // Append arrays properly
            data.append('tags', JSON.stringify(completeData.tags));
            data.append('categories', JSON.stringify(completeData.categories));

            // Additional business info (stringify object if needed or flatten)
            const businessInfo = {
                type: completeData.businessType,
                regNo: completeData.registrationNumber,
                est: completeData.yearEstablished,
                contact: completeData.contactPhone
            };
            data.append('businessInfo', JSON.stringify(businessInfo));

            // TODO: Add image upload to onboarding UI eventually. 
            // For now backend uses default or we could add file input here.

            await api.shopOwner.createShop(data);

            // Fetch shop and update context immediately
            await fetchShop();
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 p-6 pb-32 overflow-auto">
            <div className="max-w-2xl mx-auto pb-8">
                ...
                <div className="mb-8">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                        <div className="bg-purple-600 h-2 rounded-full transition-all" style={{ width: '100%' }}></div>
                    </div>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {formData.categories && CATEGORIES.map(category => (
                                    <button
                                        key={category}
                                        type="button"
